...
if uploaded:
    try:
        ...
        def normalize_columns(df):
            ...
            mapping = {'Server':'server', 'Parent fleet':'parent_fleet', 'Fleet number':'fleet_number', 'Fleet number ':'fleet_number', 'Registration':'registration', 'Status':'status', 'Comments':'comments', 'Date created':'date_created', 'Date created ':'date_created', 'Priority':'priority', 'serial_number':'serial_number', 'Repair_Status':'status'}
            df_cols = {}
            for c in df.columns:
                key = c.strip()
                if key in mapping:
